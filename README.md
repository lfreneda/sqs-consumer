...
* By default messages are processed one at a time. To process messages in parallel, use the `concurrency` option [detailed below](#options).
...
* `concurrency` - _Number_ - The maximum number of messages that can be processed concurently (default `1`).
